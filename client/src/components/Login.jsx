...
export default function Login() {
    ...
    const handleResetPassword = async (e) => {
        ...
        try {
            ...
            if (error) throw error;
            showMsg('✅ Email enviado. Revisá tu casilla para restablecer la contraseña.', 'success');
        } catch (error) {
            console.error('Reset Error:', error);
            showMsg(error.message || 'Error al intentar enviar el email de recuperación.');
        } finally {
            setLoading(false);
        }
    };
            ...
            if (isSignUp) {
                ...
                try {
                    ...
                    if (resp.ok) {
                        ...
                        localStorage.setItem('alex_io_role', backendData.role || 'OWNER');
                        localStorage.setItem('alex_io_tenant', backendData.tenantId || data.user.id);

                        if (!rememberSession) {
                            sessionStorage.setItem('alex_io_token', backendData.token);
                            localStorage.removeItem('alex_io_token');
                        }

                        navigate((backendData.role === 'SUPERADMIN') ? '/superadmin' : '/dashboard');
                    } else {
                        const errData = await resp.json();
                        throw new Error(errData.error || 'Fallo en la validación del backend');
                    }
                } catch (fetchErr) {
                    // Fallback para evitar bloqueo total si el backend tarda en arrancar
                    console.error('❌ Backend Session Error:', fetchErr.message);
                    showMsg('Conectando con el servidor... por favor intentá de nuevo en un minuto.');
                    return;
                }
            }
        } catch (error) {
        ...
        <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden">
            ...
            >
                ...
                <h1 className="text-3xl font-bold text-white mb-2 text-center tracking-tight">
                    {isReset ? 'Recuperar Cuenta' : isSignUp ? 'Crear Cuenta' : 'Bienvenido'}
                </h1>
                <p className="text-slate-500 text-sm text-center mb-6">
                    {isReset ? 'Ingresá tu email para continuar' : isSignUp ? 'Registrate para empezar' : 'Ingresá a tu cuenta'}
                </p>
                ...
                <form onSubmit={isReset ? handleResetPassword : handleAuth} className="space-y-4">
                    ...
                    {!isReset && (
                        <div>
                            <div className="flex justify-between items-center mb-2">
                                <label className="block text-slate-400 text-xs font-bold uppercase tracking-wider">Contraseña</label>
                                {!isSignUp && (
                                    <button
                                        type="button"
                                        onClick={() => { setIsReset(true); setMessage(''); }}
                                        className="text-cyan-500 hover:text-cyan-400 text-[10px] font-bold uppercase tracking-tighter"
                                    >
                    ...
                    >
                        ...
                        <span>{isReset ? 'Enviar Instrucciones' : isSignUp ? 'Crear Cuenta' : 'Entrar'}</span>
                    </button>
                    ...
                    {isReset && (
                        <button
                            ...
                            className="w-full py-2 text-slate-400 hover:text-white text-sm transition-colors mt-2"
                        >
