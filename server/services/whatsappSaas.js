...
const updateSessionStatus = async (instanceId, status, extra = {}) => {
    const payload = {
        ...
        value: JSON.stringify(extra.fullConfig || {}), // Persist full config JSON
        status,
...
router.post('/connect', async (req, res) => {
    ...
    const ALEX_MODE = process.env.ALEX_MODE || 'ENTERPRISE';

    if (ALEX_MODE === 'FREE' && provider !== 'baileys') {
        return res.status(403).json({ error: 'Este canal (Meta/Cloud) no está disponible en la versión Free. Pasa al plan Premium.' });
    }
...
router.post('/config/:instanceId', async (req, res) => {
    ...
    // Log intent to update
    console.log(`⚙️ [${instanceId}] Configuración actualizada para ${nextConfig.companyName}`);

    await updateSessionStatus(instanceId, 'configured', {
        companyName: nextConfig.companyName,
        provider: nextConfig.provider,
        qr_code: null,
        fullConfig: nextConfig // Pass full config for persistence
    });
    ...
    if (body.object) {
        ...
        if (messages && messages[0]) {
            ...
            if (text) {
                const ALEX_MODE = process.env.ALEX_MODE || 'ENTERPRISE';
                if (ALEX_MODE === 'FREE') {
                    console.warn(`🛑 [Cloud Webhook] Bloqueado en Modo FREE.`);
